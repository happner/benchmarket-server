...
module.exports.store = function(params) {
  ...
  return function(metric) {
    ...
    */
    ...
    var runsRow, hostRow, repoRow, branchRow, testfileRow, testRow;

    return Promise.resolve() ////////////// TODO, transaction

    .then(function() {
      return module.exports.updateHost(user, conn, metric);
    })
    ...
    .then(function(repo) {
      repoRow = repo;
      return module.exports.updateBranch(user, conn, metric, repoRow);
    })

    .then(function(branch) {
      branchRow = branch;
      return module.exports.updateRun(user, conn, metric, hostRow, repoRow, branchRow);
    })
...
module.exports.updateHost = function(user, conn, metric) {
  return new Promise(function(resolve, reject) {
    conn.query('SELECT id FROM hosts WHERE users_id = $1 AND name = $2',
    [user.id, metric.host],
    function(err, hostResult) {
      if (err) return reject(err);

      if (hostResult.rows.length === 0) {
        return conn.query('INSERT INTO hosts (created_at, users_id, name) VALUES ($1, $2, $3)',
       [(new Date()).toISOString(), user.id, metric.host],
        function(err, result) {
          if (err) return reject(err);
...
module.exports.updateBranch = function(user, conn, metric, repoRow) {
  return new Promise(function(resolve, reject) {
    conn.query('SELECT id FROM branches WHERE repositories_id = $1 AND name = $2',
    [repoRow.id, metric.branch],
    function(err, branchResult) {
      if (err) return reject(err);

      if (branchResult.rows.length === 0) {
        return conn.query('INSERT INTO branches (created_at, repositories_id, name) VALUES ($1, $2, $3)',
       [(new Date()).toISOString(), repoRow.id, metric.branch],
        function(err, result) {
          if (err) return reject(err);
      ...
      resolve(branchResult.rows[0]);
    });
  });
}


module.exports.updateRun = function(user, conn, metric, hostRow, repoRow, branchRow) {
  return new Promise(function(resolve, reject) {
    ...
    function(err, runResult) {
      ...
      if (runResult.rows.length === 0) {
        return conn.query('INSERT INTO runs (created_at, run_ref, users_id, repositories_id, branches_id, hosts_id) VALUES ($1, $2, $3, $4, $5, $6)',
        [metric.ts, metric.ref, user.id, repoRow.id, branchRow.id, hostRow.id],
        function(err, result) {
          if (err) return reject(err);
